...
export default function ColorSetPreview({ colorInfos, onClose }: ColorSetPreviewProps) {
        ...
        {colorInfos.map((colorInfo, index) => (
          ...
          >
            <div className="bg-black/70 backdrop-blur-md text-white px-6 py-4 rounded-lg">
              <div className="text-2xl font-semibold mb-1">{colorInfo.name}</div>
              <div className="text-sm font-extralight opacity-80">{colorInfo.code}</div>
            </div>
          </div>
        ))}
      </div>
